        ...
        if (len == 2) {
            ...
            if (value == arr[midIdx]) {
                ...
                if (value < arr[midIdx]) {
                    ...
                    retIdx = BinarySearch(newArray, value, idxOff); // FIX: Pass idxOff instead of 0 to maintain the correct offset.
                } else if (arr[midIdx] < value) {
                    int[] newArray = Arrays.copyOfRange(arr, midIdx + 1, max); // FIX: Adjust the range to exclude the midIdx.
                    idxOff += midIdx + 1; // FIX: Update the idxOff with the correct offset.
